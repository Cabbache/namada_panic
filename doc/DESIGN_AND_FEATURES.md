...
Different events vary in severity. We cannot treat an alert for a new version of Namada as being on the same level as an alert for validator downtime. PANIC makes use of four alert types:
...
Telegram bots in PANIC serve two purposes. As mentioned above, they are used to send alerts. However they can also accept commands that allow you to check the status of the alerter (and its running monitors), snooze or unsnooze calls, mute or unmute periodic alive reminders.
